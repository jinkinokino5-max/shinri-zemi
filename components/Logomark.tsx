/* ══════════════════════════════════════════════════════════════════
   題字「北大心理ゼミ」
   根拠：draft/デザイン大本資料_v1.md 4-2-1

   書体：キルゴU（KanaNA）／作者：残雪・GN's Side
        (C)2014-2015 Getsuren/Nagoriyuki ／ http://getsuren.com/

   ⚠ フォントファイルは同梱も配信もしない。
     同梱 readme.txt にこうある（原文）：
       「Web上でこのフォントを二次的に販売したりする事や、
         フォントファイル自体を無許可で再配布・改変する事はご遠慮下さい。
         (個人間での配布・アウトラインデータ化してからなどの改変は自由です)」
     → Webフォント設置は「再配布」、サブセット化は「改変」にあたる。
       一方アウトライン化は明示的に許諾されている。
       よって「北大心理ゼミ」6字をアウトライン化したSVGパスとして持つ。

   ⚠ 公開時、作者へ一報を入れること（readmeが希望している）。
     gn@getsuren.com ／ X: @snowy_tgn
...
/**
 * 題字。読み上げ・SEO用の実テキストを必ず伴う。
 * ⚠ musabi のサイトは <h1> が空（画像で組んでいた）という実測結果がある。
 *   同じ失敗をしないため、視覚的に隠したテキストを併記する。
 */
export function Logomark({ className }: { className?: string }) {
  return (
    <>
      <svg className={`logomark ${className ?? ''}`} aria-hidden="true">
        <use href="#logomark" />
      </svg>
      <span className="vh">北大心理ゼミ</span>
    </>
  );
}
